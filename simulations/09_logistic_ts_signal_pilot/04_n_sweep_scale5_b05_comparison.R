# ======================================================================
# Logistic n-sweep comparison for TS-RIGHT / IHT / Lasso / Shrinkage
#
# Goal:
#   Comparison experiment at the promising setting found in the pilot:
#   df_X = 2.1, scale_X = 5, theta_magnitude = 0.50.
#   The scientific axis in this script is n.
#
# Main metric:
#   ||theta_hat - theta_star||_2
#
# Outputs:
#   results/09_logistic_ts_signal_pilot/n_sweep_scale5_b05/raw/
#   results/09_logistic_ts_signal_pilot/n_sweep_scale5_b05/summary/
#   results/09_logistic_ts_signal_pilot/n_sweep_scale5_b05/figures/
# ======================================================================
...
cfg <- list(
  experiment_id = "logistic_n_sweep_scale5_b05",
  
  p = 800,
  # n = 600 is not eligible under q = 0.5, T2 = 8, m_min = 15, c_K2 = 0.50:
  # b2 = floor(0.5 * 600 / 8) = 37, so floor(b2 / m_min) < 4.
  # Keep the promising schedule fixed and start the n-sweep at 1000.
  n_grid = c(1000, 1400, 1800, 2200),
  s_star = 10,
  s = 20,
  init_bad_value = 5,
  m_min = 15,
  ...
  shrink_tau_quantile_grid = c(0.85, 0.90, 0.95),
...
cfg$init_bad_value <- env_num_vec("PILOT_INIT_BAD_VALUE", cfg$init_bad_value)[1]
...
out_dir <- here("results", "09_logistic_ts_signal_pilot", "n_sweep_scale5_b05")
...
make_bad_theta_init <- function(p, s_star, value = 5) {
  theta_init <- rep(0, p)
  wrong_support <- (s_star + 1L):min(p, 2L * s_star + 1L)
  theta_init[wrong_support] <- value
  theta_init
}
...
fit_iht_logistic_theta <- function(X, y, s, eta, T, theta_init) {
...
fit_one_rep <- function(n, n_index, theta_magnitude, mag_index, rep_id, cfg) {
  ...
  theta_init <- make_bad_theta_init(
    p = cfg$p,
    s_star = cfg$s_star,
    value = cfg$init_bad_value
  )
    ...
    for (T1 in cfg$T1_grid) {
      for (T2 in cfg$T2_grid) {
        ...
        rows[[length(rows) + 1L]] <- data.frame(
          ...
          init_bad_value = cfg$init_bad_value,
          init_l2_error = safe_l2(theta_init, theta_star),
          seed_data = seed_data,
          seed_algo = seed_ts
        )
      }
  ...
  rows[[length(rows) + 1L]] <- data.frame(
    ...
    K1_block_size = NA_real_,
    K2_block_size = NA_real_,
    init_bad_value = NA_real_,
    init_l2_error = NA_real_,
    seed_data = seed_data,
...
message("Running logistic n-sweep comparison: ", nrow(param_grid), " data replications.")
...
message("Hard-thresholding init: coordinates s*+1 through 2s*+1 set to ", cfg$init_bad_value)

helper_exports <- c(
  "cfg", "param_grid", "seed_for", "with_temp_seed", "restore_temp_seed",
  "make_theta_star_signed", "make_bad_theta_init", "make_foldid", "safe_l2", "logistic_nll",
...
saveRDS(raw_results, file.path(raw_dir, "raw_n_sweep_scale5_b05_logistic.rds"))
write.csv(raw_results, file.path(raw_dir, "raw_n_sweep_scale5_b05_logistic.csv"), row.names = FALSE)
...
write.csv(summary_results, file.path(summary_dir, "summary_n_sweep_scale5_b05_logistic.csv"), row.names = FALSE)
saveRDS(summary_results, file.path(summary_dir, "summary_n_sweep_scale5_b05_logistic.rds"))
...
plot_median <- summary_results %>%
  ...
  ggplot(aes(x = n, y = median_l2, color = method, shape = method, group = method)) +
  geom_line(linewidth = 0.9) +
  geom_point(size = 2.5) +
  scale_y_log10() +
  scale_x_continuous(breaks = cfg$n_grid) +
  labs(
    x = "Sample size n",
...
print(plot_median)
ggsave(file.path(fig_dir, "logistic_n_sweep_scale5_b05_median_l2.pdf"), plot_median, width = 8.5, height = 5)
ggsave(file.path(fig_dir, "logistic_n_sweep_scale5_b05_median_l2.png"), plot_median, width = 8.5, height = 5, dpi = 300)

plot_eta <- summary_results %>%
  distinct(n, theta_magnitude, median_eta_sd, median_eta_abs, median_eta_gt3_rate, median_bernoulli_var) %>%
  ggplot(aes(x = n, y = median_eta_sd)) +
  geom_line(linewidth = 0.9) +
  geom_point(size = 2.3) +
  scale_x_continuous(breaks = cfg$n_grid) +
  labs(
...
print(plot_eta)
ggsave(file.path(fig_dir, "logistic_n_sweep_scale5_b05_eta_sd.pdf"), plot_eta, width = 7, height = 4.5)
